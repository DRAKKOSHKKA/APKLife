...
def _parse_lesson_div(div, lesson_time):
...
def parse_schedule_html(html_text):
    ...
    for th in header_rows[0].find_all("th")[1:]:
        time_headers.extend([th.get_text(strip=True)] * int(th.get("colspan", 1)))
    ...
    for row in soup.select("tbody tr"):
        ...
        for td in row.find_all("td"):
            ...
            if lesson_divs:
                ...
                lessons = [
                    _parse_lesson_div(div, time_intervals[idx] if idx < len(time_intervals) else current_time_range)
        ...
        for idx, cell in enumerate(all_cells):
            ...
            if cell["lessons"]:
                ...
                empty = dict(EMPTY_LESSON)
                empty["time"] = cell["time_range"]
