...
def get_current_week() -> int:
    ...
    return ((today - epoch).days // 7) + 11660


def get_academic_week_number(week_id: int) -> int:
    """
    Вычисляет номер учебной недели по формуле сайта it-institut.ru.
    WeekId 14583 = неделя 1, WeekId 14601 = неделя 19.
    Формула: week_id - 14582.
    """
    return max(week_id - 14582, 1)
...
def fetch_network_schedule(week_id: int, entity_info: dict[str, object]):
    ...
    try:
        ...
        parsed = source.parse(html, week_id)
