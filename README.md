...
Если возникает `WinError 2` при запуске `npm/npx`, убедитесь, что в `PATH` доступны `npm.cmd` и `npx.cmd` (обычно ставятся вместе с Node.js). Скрипт `bridge.py` теперь проверяет и использует именно эти пути на Windows автоматически.
